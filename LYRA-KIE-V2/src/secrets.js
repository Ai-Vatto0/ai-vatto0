...
export async function saveKieApiKey(value) {
  const key = value?.trim(); if (!key || key.length < 12) throw new Error('Der KIE API Key ist leer oder unplausibel kurz.');
  const enc = encryptSecret(key); const sql = await db();
...
export async function loadKieApiKey() {
  const sql = await db(); const rows = await sql`SELECT ciphertext,iv,auth_tag FROM lyra_secrets WHERE secret_name='kie_api_key' LIMIT 1`;
  if (!rows[0]) return null; return decryptSecret(rows[0]);
}
export async function kieKeyStatus() {
  if (process.env.KIE_API_KEY?.trim()) return { configured: true, source: 'environment' };
  try {
    const sql = await db(); const rows = await sql`SELECT updated_at FROM lyra_secrets WHERE secret_name='kie_api_key' LIMIT 1`;
    return { configured: Boolean(rows[0]), source: rows[0] ? 'encrypted-database' : null, updated_at: rows[0]?.updated_at ?? null };
  } catch (e) { return { configured: false, error: e instanceof Error ? e.message : String(e) }; }
}
