...
export async function databaseHealth() {
  ...
  try {
    ...
    return { mode: "neon-postgres", persistent: true, ok: false, error: e instanceof Error ? e.message : String(e) };
  }
}
...
export async function getProduct(id) {
  if (!configured()) return mem().products[id] ?? null;
  const db = sql(); const rows = await db`SELECT * FROM lyra_product_cheats WHERE product_id=${id} LIMIT 1`; return rows[0] ?? null;
}
export async function listProducts() {
  if (!configured()) return Object.values(mem().products).sort((a,b)=>String(b.updated_at).localeCompare(String(a.updated_at)));
  const db = sql(); return await db`SELECT * FROM lyra_product_cheats ORDER BY updated_at DESC`;
}
...
export async function getJob(id) {
  if (!configured()) return mem().jobs[id] ?? null;
  const db=sql(); const rows=await db`SELECT * FROM lyra_render_jobs WHERE task_id=${id} LIMIT 1`; return rows[0]??null;
}
export async function listJobs(limit=20) {
  if (!configured()) return Object.values(mem().jobs).sort((a,b)=>String(b.updated_at).localeCompare(String(a.updated_at))).slice(0,limit);
  const db=sql(); return await db`SELECT * FROM lyra_render_jobs ORDER BY updated_at DESC LIMIT ${limit}`;
}
